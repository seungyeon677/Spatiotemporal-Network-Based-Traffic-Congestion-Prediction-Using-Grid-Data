...
class TGCN_encoder(nn.Module):
    ...
    def create_update_gate(self):
        self.conv_z = ChebConv(in_channels = self.in_channels, out_channels = self.out_channels, K = self.K)
        self.linear_z = nn.Linear(2 * self.out_channels, self.out_channels)
    
    def create_reset_gate(self):
        self.conv_r = ChebConv(in_channels=self.in_channels, out_channels=self.out_channels, K = self.K)
        self.linear_r = nn.Linear(2 * self.out_channels, self.out_channels)

    def create_candidate_state(self):
        self.conv_h = ChebConv(in_channels=self.in_channels, out_channels=self.out_channels, K = self.K)
...
class TGCN(nn.Module):
    def __init__(self, in_channels, out_channels, K):
        ...
        self.tgcn_encoder = TGCN_encoder(in_channels, out_channels, K)
...
def objective(trial):
    ...
    model = TGCN(in_channels, out_channels, K).to(device)
...
for min in min_list:
    ...
    # txt 파일로 저장
    with open(cmd + '025_TGCN-C_test_result2/best_params'+ str(min) + '.txt', "w") as f:
    ...
    plt.ylabel('loss')
    plt.xlabel('epoch')
    plt.savefig(cmd + '025_TGCN-C_test_result2/025_TGCN-C_loss_graph' + str(min) + '.jpg', dpi = 600)
    # plt.show() 
    ...
    plt.legend()
    plt.savefig(cmd + '025_TGCN-C_test_result2/025_TGCN-C_sample' + str(min) + '.jpg', dpi = 600)
    # plt.show()

    a = np.array(actuals)
    p = np.array(predictions)
    np.save(cmd + '025_TGCN-C_test_result2/actual' + str(min) + '.npy', a)
    np.save(cmd + '025_TGCN-C_test_result2/prediction' + str(min) + '.npy', p)


df = pd.DataFrame()
df['MSE'] = min_mse
df['MAE'] = min_mae
df.to_csv(cmd + '025_TGCN-C_test_result2/Accuracy_result.csv', index = False)
